...
class Planner(object):
	...
	def callback(self, msg):
		...
		if self.PLAN:
		    ...
		    if self.RobotPose.pathDistance < 1:
		        robotVel = 0.5
		        K = 0
		    ...
		    # Straight line back
		    elif self.RobotPose.pathDistance < 1+.25*np.pi+1:
		        robotVel = 0.5
		        K = 0
		        desiredV =self.PathPlanner(robotVel, K)
	...
	def calc_dPhis(self, leftEnc, rightEnc):
		...
		dPhiR = self.enc2rad*rightEnc
		dPhiL = self.enc2rad*leftEnc
	...
	def PathPlanner(self, robotVel, K):
        ...
        b = self.b
        V = robotVel
		desiredWV_L = V/r*(1-b*K)
		desiredWV_R = 2*b*V*K/r+desiredWV_L
